  ...
  module Management
    ...
    class ServiceAccounts < Grape::API
      ...
      resource :service_accounts do
        ...
        post '/list' do
          service_accs = ServiceAccount.all
        ...
        params do
          requires :service_account_uid, type: String, allow_blank: false, desc: 'service_account uid'
          requires :provider_uid, type: String, allow_blank: false, desc: 'provider uid'
          requires :service_account_email, type: String, allow_blank: false, desc: 'service_account email'
        end
        post '/create' do
          provider = User.find_by(uid: params[:provider_uid])
          error!('Provider doesnt exist', 422) unless provider

          service_acc = ServiceAccount.new(email: params[:service_account_email], uid: params[:service_account_uid], user: provider)
